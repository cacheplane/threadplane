...
- Deviation from spec, recorded: reported rows are written after provider acceptance and before job completion rather than in one transaction with completion; the at-most-once claim gives the same no-double-send and no-double-report guarantee, matching the notify job.
