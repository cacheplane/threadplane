...
| `marketing:pre_analytics_exit` | A visitor left **before PostHog initialized**, so posthog-js never recorded them at all. Sent by an inline beacon (`apps/website/src/lib/analytics/pre-analytics-beacon.ts`), not by posthog-js; `elapsed_ms` (since navigation start), `trigger` (`pagehide` = definite leave; `hidden` = backgrounded, usually a phone leaving, but the visitor may return), `source_page` (pathname only), `referrer_host`, `viewport_width`, `effective_type` (Chromium only, else null). Random `distinct_id`, no person profile, no IP. Deliberately **not** a `$pageview`: these visits are absent from bounce rate and sessions, which remain computed only from visitors whose connections were fast enough for PostHog to load. Read it alongside them to size that missing cohort, not merged into them. |
